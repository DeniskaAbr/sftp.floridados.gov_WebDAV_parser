...
func (c *Client) NewRequest(method string, path string, body io.Reader) (*http.Request, error) {
	req, err := http.NewRequest(method, c.ResolveHref(path).String(), body)

	if err != nil {
		return nil, err
	}

	// INFO: hardcoded by Me
	req.Header.Add("User-Agent", "WinSCP/5.17.7 neon/0.31.1")
	req.Header.Add("Authorization", "Basic UHVibGljOlB1YkFjY2VzczE4NDUh")

	// req.Header.Add("Connection", "TE")
